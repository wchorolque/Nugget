...
class SmsSendGui(gtk.Table):
    def __init__(self, smsgui):
        ...
        self._btnPreview = gtk.Button('Vista previa')
        self._btnPreview.connect('clicked', self.on_preview_clicked)
        bbox.pack_end(self._btnShort)
        bbox.pack_end(self._btnPreview)
        bbox.set_spacing(5)
